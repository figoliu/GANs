...
def lr_images(images_real , downscale):
    ...
    for img in  range(len(images_real)):
        images.append(transform.imresize(images_real[img], [images_real[img].shape[0]//downscale,images_real[img].shape[1]//downscale], interp='bicubic', mode=None))
...
def load_training_data(directory, ext, train_test_ratio = 0.8):
    
    files = load_data_from_dirs(load_path(directory), ext)
    number_of_images = len(files)
    number_of_train_images = int(number_of_images * train_test_ratio)
    print("Load %d images, train set have %d images" %[number_of_images, number_of_train_images])
